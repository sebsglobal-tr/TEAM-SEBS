...
export function ManagerWorkSessions() {
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="page-subtitle">Ekibinizdeki çalışanların çalışma süreleri</p>
      </div>
      ...
      <div className="filters-bar">
        ...
        <button className="btn btn-primary" onClick={() => load(startDate, endDate)} style={{ marginTop: 22 }}>Getir</button>
      </div>
      ...
      {sessions.length === 0 ? (
        ...
        <div className="table-responsive">
          <table className="table">
            ...
            <tbody>
              {sessions.map((s) => {
                const total = s.totalActive + s.totalIdle + s.totalBreak;
                return (
                  <tr key={s.userId} style={{ cursor: 'pointer' }} onClick={() => navigate(`/manager/employees/${s.userId}`)}>
